...
static PyObject *my_strptime(PyObject *self, PyObject *args) {
  const char *datetime;
  ...
  if (extraneous_chars == NULL) {
    PyErr_SetString(PyExc_ValueError,
                    "Date/time format did not match format string");
    return (PyObject *)NULL;
  }

  py_datetime =
      PyDateTime_FromDateAndTime(tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
static PyMethodDef StrpMethods[] = {
    {"strptime", my_strptime, METH_VARARGS, "Parse a time string."},
    {NULL, NULL, 0, NULL} /* Sentinel */
};

static struct PyModuleDef strptimemodule = {
    PyModuleDef_HEAD_INIT, "strptime",
    "A module for parsing date/time values much faster.", -1, StrpMethods};
